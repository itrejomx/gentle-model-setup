...
On the maintainer's go-ahead push `feat/2-cli-ci-error-surfaces` and open PR #29 against `feat/2-bundle-hardening`; T10.9 closes when the CI workflow is green on that PR. Then decide which slice 12 review findings become a follow-up slice or issue, and run close-out TC.1, TC.3, TC.4, and TC.2 (archive, needs explicit go-ahead).
